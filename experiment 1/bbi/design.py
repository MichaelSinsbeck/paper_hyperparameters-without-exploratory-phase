...
def design_linearized(problem, field, n_iterations, nodes=None):
    ...
    for i_iteration in range(n_iterations):
        print('.', end='')
        new_index = find_optimal_node_linear(nodes, this_field, problem.data)
...
def find_optimal_node_linear(nodes, field, data):
...
def compute_criterion_linear(index_list, field, data):
...
def design_map(problem, fields, n_iterations, nodes=None):
    print('design_map          ', end='')
    if nodes is None:
        nodes = Nodes()
    n_eval = np.arange(n_iterations+1) + nodes.idx.size

    this_prior_field = fields.get_map_field(nodes)
    this_field = this_prior_field.condition_to(nodes)
...
def compute_average_criterion(index_list, fields, data):
    ...
    idx_ok = np.full(fields.n_fields, False) # keep track if there are nan values in the array
    for i, subfield in enumerate(fields.subfields):
        ...
        idx_ok[i] = not np.any(np.isnan(this_criterion))
    return criterion[:,idx_ok] @ fields.weights[idx_ok]
...
def design_random(problem, field, n_iterations, nodes=None):
    ...
    for i_iteration in range(n_iterations):
        print('.', end='')
        # choose random point
        new_index = pick_random_node_without_duplicates(
            this_field.n_sample, nodes)
...
def find_heuristic_node(this_field, data):
    var = np.diag(this_field.c)
    l = this_field.estimate_likelihood(data)
    criterion = np.sqrt(var)*l
    new_index = np.argmax(criterion)
    return new_index
